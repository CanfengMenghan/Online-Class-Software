...
void Teacherop::get_window_title_list(vector<pair<QString, HWND> >& list)
{
    ...
    this->teacherclient->get_window_title_list(stdlist);
...
void Teacherop::send_vid(QByteArray& data)
{
    ...
    this->teacherclient->send_vid(msg);
}
...
{
    this->teacherclient->add_record(username, start_time, quit_time, att_ratio);
}
...
{
    for (int i = 0; i < this->teacherclient->records.size(); i++) {
        Record* rec = this->teacherclient->records[i];
